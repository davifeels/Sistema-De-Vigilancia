...
# !!!!!!! VERSÃO DE TESTE TEMPORÁRIA PARA IGNORAR O ERRO DO YOLO !!!!!!!

import sys
import os
import json
# import cv2 # DESABILITADO
import time
...
# from detector import Detector # <--- ESSA LINHA ESTÁ DANDO ERRO
...
# ====================================================================================
QSS_STYLE = """
/* ... (Seu QSS continua o mesmo, não vou repetir aqui para economizar espaço) ... */
...
QTabBar::tab { background-color: #2D2D2D; color: white; padding: 10px 25px; border-top-left-radius: 5px; border-top-right-radius: 5px; border: 1px solid #444; }
QTabBar::tab:selected { background-color: #007ACC; border-bottom-color: #007ACC; }
QTabBar::tab:!selected:hover { background-color: #3E3E3E; }
CameraWidget { background-color: #2D2D2D; border-radius: 10px; }
#CameraTitle { background-color: #007ACC; color: white; font-size: 14px; font-weight: bold; padding: 5px; border-top-left-radius: 10px; border-top-right-radius: 10px; }
#VideoLabel { background-color: black; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px; }
QListWidget, QTableWidget { background-color: #2D2D2D; border-radius: 5px; border: 1px solid #444; font-size: 14px; alternate-background-color: #3E3E3E; }
QHeaderView::section { background-color: #007ACC; color: white; padding: 5px; border: 1px solid #444; }
QListWidget::item { padding: 8px; }
QListWidget::item:selected, QTableWidget::item:selected { background-color: #007ACC; color: white; }
QPushButton { background-color: #007ACC; color: white; border: none; padding: 10px 20px; border-radius: 5px; font-size: 14px; font-weight: bold; }
QPushButton:hover { background-color: #005A9E; }
QDialog { background-color: #2D2D2D; }
QLineEdit { background-color: #3E3E3E; color: white; border: 1px solid #555; border-radius: 5px; padding: 5px; font-size: 14px; }
"""
...
class CameraWidget(QWidget):
    ...
    def __init__(self, camera_id, nome_camera, placeholder_img, parent=None):
        ...
        self.label_nome = QLabel(f"{nome_camera} (Desabilitado - Erro YOLO)"); self.label_nome.setObjectName("CameraTitle"); self.label_nome.setAlignment(Qt.AlignmentFlag.AlignCenter); self.layout.addWidget(self.label_nome)
        self.label_video = QLabel(self); self.label_video.setObjectName("VideoLabel"); self.layout.addWidget(self.label_video)
        
        # Coloca um placeholder de "sem sinal"
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Phoenix Vision - Painel de Vigilância (MODO DE TESTE)")
    ...
    def init_ui(self):
        ...
        self.tab_widget.addTab(self.camera_panel, "Painel de Câmeras (DESABILITADO)")
        ...
        self.face_gallery_panel = FaceGalleryWidget()
        ...
        for position, cam_config in zip(positions, self.settings['cameras']):
            # Usamos o CameraWidget "capado"
            widget = CameraWidget(cam_config['id'], cam_config['name'], cam_config['placeholder'])
            # widget.doubleClicked.connect(lambda w=widget: self.toggle_fullscreen(w)) # Desabilitado
    ...
    def closeEvent(self, event):
        ...
        # Precisamos garantir que a câmera da galeria de rostos pare
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Tenta mostrar o erro em uma caixa de diálogo como último recurso
        QMessageBox.critical(None, "Erro Crítico na Inicialização",
                             f"Ocorreu um erro fatal ao iniciar o aplicativo:\n\n{str(e)}\n\n"
